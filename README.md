...
- **Text detection (beta)**: 3-layer ensemble — L1 perplexity (GPT-2) + L2 stylometry (spaCy) + L3 DeBERTa-v3-small fine-tuned on an adversarial-augmented corpus. Current `MODEL_VERSION = "3.2-g2-removed-product-output"`. A reliability-gated 3-zone decision policy (AI `≥ 0.70`, HUMAN `≤ 0.30`, else UNCERTAIN) plus a G1 short-text gate (`< 50` words → UNCERTAIN) sit on top of the combiner; the legacy G2 L2-L3 disagreement gate is deprecated and intentionally disabled. End-to-end F1 on held-out test splits, combiner-only (no reliability gating): **0.9945** (v1, n=2000) and **0.9529** (v2 with adversarial subsets, n=3482). Reliability-gated metrics (definitive predictions only) with current G2-OFF config: v1 reliability 0.9971 / coverage 86.0% / F1 0.9965; v2 reliability 0.9514 / coverage 99.80% / F1 0.9526. Combiner weights and AI threshold are fit on validation data. Full numbers and reproducibility recipe: [ai/text_detector/ACCURACY.md](ai/text_detector/ACCURACY.md). L4 adversarial and the XGBoost meta-classifier exist in code but are not loaded in production.
